...
public class RedisUtils {
    ...
    public Object get(String key){
        return key==null?null:rt.opsForValue().get(key);
    }
    /**
     * 普通缓存放入
     * @param key 键
    ...
    public boolean set(String key,Object value,long time){
        try {
            if(time>0){
                rt.opsForValue().set(key, value, time, TimeUnit.MINUTES);
            }else{
                set(key, value);
            }
            return true;
    ...
    public Object hget(String key,String item){
        return rt.opsForHash().get(key, item);
    }
    /**
     * 获取hashKey对应的所有键值
     * @param key 键
    ...
    public boolean hHasKey(String key, String item){
        return rt.opsForHash().hasKey(key, item);
    }
    /**
    ...
    public double hincr(String key, String item,double by){
        return rt.opsForHash().increment(key, item, by);
    }
    /**
     * hash递减
     * @param key 键
    ...
    public long sSet(String key, Object...values) {
        try {
            return rt.opsForSet().add(key, values);
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
    ...
    public Object lGetIndex(String key,long index){
        try {
            return rt.opsForList().index(key, index);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public long lRemove(String key,long count,Object value) {
        try {
            Long remove = rt.opsForList().remove(key, count, value);
            return remove;
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
    ...
    public List<Object> rangeList(String listKey, long start, long end) {
        //绑定操作
        BoundListOperations<String, Object> boundValueOperations = rt.boundListOps(listKey);
        //查询数据
